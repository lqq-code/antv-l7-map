...
function analyzeDeps(raw, {
  ...
  if (!cache) {
    ...
    const _babel$transformSync = babel().transformSync(raw, (0, _options.getBabelOptions)({
      ...
      transformRuntime: false
          ...
          ast = _babel$transformSync.ast; // traverse all require call expression
      ...
      CallExpression(callPath) {
        ...
        if (types().isIdentifier(callPathNode.callee) && callPathNode.callee.name === 'require' && types().isStringLiteral(callPathNode.arguments[0])) {
          ...
          if (resolvePath.includes('node_modules') || isHostPkg) {
            ...
            const peerDeps = []; // process peer dependencies from dependency
          ...
          resolvePath !== (0, _slash().default)(entryAbsPath || '') && // to avoid collect alias module
  ...
  cache.files.filter(item => {
    ...
    cachers.content.add(item.resolvePath, files[item.filename]); // continue to collect deps for dep
...
function getCSSForDep(dep) {
  ...
  `${pkgWithoutGroup}`, // @group/pkg-suffix => pkgsuffix @ant-design/pro-card => card
  ...(pkgWithoutGroup.includes('-') ? [pkgWithoutGroup.replace(/-/g, ''), pkgWithoutGroup.split('-')[1]] : []), // guess normal css files
